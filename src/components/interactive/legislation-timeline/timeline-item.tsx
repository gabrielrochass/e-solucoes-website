...
/**
 * Card de altura fixa; o resumo expande como overlay em hover/focus —
 * vizinhos nunca se movem (zero layout shift).
 */
export function TimelineItem({
  ...
  return (
    <li
      ...
      tabIndex={tabIndex}
      onFocus={onFocus}
      aria-label={`${event.year}, ${event.nr}: ${event.title}`}
      className="group relative h-56 w-72 shrink-0 snap-start overflow-hidden rounded-lg bg-surface-raised p-card shadow-card outline-offset-2"
    >
      <p className="text-stat tabular-stat text-petrol-200 transition-colors group-focus-within:text-petrol-300 group-hover:text-petrol-300">
        {event.year}
      </p>
